...
#define pr_fmt(fmt) "r8a66597: " fmt
	...
	if (root_port) {
		*root_port = (devpath[0] & 0x0F) - 1;
		if (*root_port >= r8a66597->max_root_hub)
			pr_err("Illegal root port number.\n");
	}
	if (hub_port)
			      ...
			      u16 mask, u16 loop)
{
	...
	do {
		...
		if (i++ > 1000000) {
			pr_err("register%lx, loop %x is timeout\n", reg, loop);
			break;
		}
		ndelay(1);
	} while ((tmp & mask) != loop);
						   ...
						   int addr)
{
	...
	pr_err("get_r8a66597_device fail.(%d)\n", addr);
	return NULL;
}
