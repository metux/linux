				...
				const struct usb_device_id *id)
{
	...
	if (request_firmware(&firmware, "isight.fw", &dev->dev) != 0) {
		pr_err("Unable to load isight firmware\n");
		ret = -ENODEV;
		goto out;
	}
	...
	if (usb_control_msg
	    (dev, usb_sndctrlpipe(dev, 0), 0xa0, 0x40, 0xe600, 0, buf, 1,
	     300) != 1) {
		pr_err("Failed to initialise isight firmware loader\n");
		ret = -ENODEV;
		goto out;
	}
	...
	while (ptr+4 <= firmware->data+firmware->size) {
		...
		for (; len > 0; req += 50) {
			...
			if (ptr+llen > firmware->data+firmware->size) {
				pr_err("Malformed isight firmware");
				ret = -ENODEV;
				goto out;
			}
			memcpy(buf, ptr, llen);
			...
			if (usb_control_msg
			    (dev, usb_sndctrlpipe(dev, 0), 0xa0, 0x40, req, 0,
			     buf, llen, 300) != llen) {
				pr_err("Failed to load isight firmware\n");
				ret = -ENODEV;
				goto out;
			}
	...
	if (usb_control_msg
	    (dev, usb_sndctrlpipe(dev, 0), 0xa0, 0x40, 0xe600, 0, buf, 1,
	     300) != 1) {
		pr_err("isight firmware loading completion failed\n");
		ret = -ENODEV;
	}
