...
static void tusb_musb_enable(struct musb *musb)
{
	...
	if (is_dma_capable() && dma_off)
		pr_warn("%s %s: dma not reactivated\n",
				__FILE__, __func__);
	else
		dma_off = 1;
}
...
static int tusb_musb_start(struct musb *musb)
{
	...
	if (ret != 0) {
		pr_err("tusb: Cannot enable TUSB6010\n");
		return ret;
	}
	...
	if (musb_readl(tbase, TUSB_PROD_TEST_RESET) !=
		...
		pr_err("tusb: Unable to detect TUSB6010\n");
		goto err;
	}
	...
	if (musb->tusb_revision < 2) {
		pr_err("tusb: Unsupported TUSB6010 revision %i\n",
				musb->tusb_revision);
		goto err;
	}
...
static int tusb_musb_init(struct musb *musb)
{
	...
	if (ret) {
		pr_err("Could not start tusb6010 (%d)\n",
				ret);
		goto done;
	}
